...
class WindowManager {
  ...
  async createWindow(type, showOnCreate = false) {
    ...
    // ── Stealth cursor fix ───────────────────────────────────────────────────
    // Intercept every cursor change at the Electron/Chromium level and force
    // it back to 'default'. This fires BEFORE the OS changes the cursor shape,
    // making it impossible for screen-share viewers to detect interaction.
    window.webContents.on('cursor-changed', (event, cursorType) => {
      if (cursorType !== 'default') {
        // Immediately execute JS in renderer to reset cursor to default
    ...
    // Also inject CSS on every page load as a secondary layer
    window.webContents.on('did-finish-load', () => {
      window.webContents.insertCSS(
        '*, *::before, *::after { cursor: default !important; }'
      ).catch(() => {});
    });
    // ── End stealth cursor fix ────────────────────────────────────────────────
