...
class LLMService {
  ...
  async processTextWithSkill(text, activeSkill, sessionMemory = [], programmingLanguage = 'java') {
  ...
  async processTranscriptionWithIntelligentResponse(text, activeSkill, sessionMemory = [], programmingLanguage = 'java') {
    if (!this.isInitialized) {
      throw new Error('LLM service not initialized. Check Gemini API key configuration.');
    }
...
Remember: Be intelligent about filtering - only provide detailed responses when the user actually needs help with ${activeSkill}.`;
   ...
   * Sends the original request + partial response back to Gemini
   * and asks it to continue from where it left off.
   * The continuation is then merged with the original for a seamless result.
   */
  async continueRequest(originalRequest, partialResponse, timeout) {
    logger.info('Starting continuation request', {
      partialResponseLength: partialResponse.length
    });

    // Build continuation request:
    // - Keep the same system instruction
    // - Add conversation history: original user message + partial assistant response
    // - Add a new user message asking it to continue
    const continuationRequest = {
      generationConfig: originalRequest.generationConfig,
      contents: []
    };

    if (originalRequest.systemInstruction) {
      continuationRequest.systemInstruction = originalRequest.systemInstruction;
    }

    // Replay the original conversation history if any
    const originalContents = originalRequest.contents || [];
    continuationRequest.contents.push(...originalContents);

    // Add the partial response as the model's last turn
    continuationRequest.contents.push({
      role: 'model',
      parts: [{ text: partialResponse }]
    });

    // Ask it to continue seamlessly
    continuationRequest.contents.push({
      role: 'user',
      parts: [{ text: 'Continue your response from exactly where you left off. Do not repeat anything already written. Do not add any preamble — just continue the response.' }]
    });

    const timeoutPromise = new Promise((_, reject) =>
      setTimeout(() => reject(new Error('Continuation request timeout')), timeout)
    );

    const requestPromise = this.model.generateContent(continuationRequest);
    const result = await Promise.race([requestPromise, timeoutPromise]);

    if (!result.response) {
      throw new Error('Empty response from continuation request');
    }
    ...
    // Check if continuation was also truncated — if so, recurse once more
    try {
      const candidates = result.response.candidates || [];
      const finishReason = (candidates[0] || {}).finishReason || 'UNKNOWN';
      const tokenCount = result.response.usageMetadata || {};

      logger.info('Continuation response metadata', {
        finishReason,
      ...
      if (finishReason === 'MAX_TOKENS') {
        // One more continuation attempt (max 2 total continuations)
        logger.warn('Continuation also truncated — attempting second continuation');
        try {
          const secondContinuation = await this.continueRequest(
            originalRequest,
            partialResponse + continuationText.trim(),
            timeout
          );
          return '' + continuationText.trim() + secondContinuation;
        } catch (e) {
          logger.warn('Second continuation failed — using first continuation only');
        }
      }
    } catch (metaError) {
      logger.debug('Could not read continuation metadata', { error: metaError.message });
    }
  ...
  async executeRequest(geminiRequest) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // Log full Gemini response metadata so we can diagnose truncation
        try {
          const candidates = result.response.candidates || [];
          ...
          logger.info('Gemini response metadata', {
            attempt,
            finishReason,                          // STOP = clean end, MAX_TOKENS = cut off
          ...
          if (finishReason === 'MAX_TOKENS') {
            logger.warn('Response truncated by token limit — attempting continuation', {
              candidatesTokenCount: tokenCount.candidatesTokenCount,
              responseLength: responseText.length
            });

            // Attempt continuation — ask Gemini to continue from where it stopped
            try {
              const continuedText = await this.continueRequest(geminiRequest, responseText.trim(), timeout);
              const finalText = responseText.trim() + continuedText;
              logger.info('Continuation successful — merged responses', {
                part1Length: responseText.length,
                part2Length: continuedText.length,
                finalLength: finalText.length
              });
              return finalText;
            } catch (contError) {
              logger.warn('Continuation failed — returning partial response', {
            ...
            logger.warn('Response was BLOCKED by safety filters');
          } else if (finishReason === 'STOP') {
            logger.info('Response completed normally (STOP)');
          }
        } catch (metaError) {
          logger.debug('Could not extract response metadata', { error: metaError.message });
        }
