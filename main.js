  ...
  setupIPCHandlers() {
    ...
    ipcMain.handle("start-speech-recognition", async () => {
      try {
        ...
        logger.error("Failed to start speech recognition via IPC", { error: error.message });
      }
    ...
    ipcMain.on("start-speech-recognition", async () => {
      try {
        ...
        logger.error("Failed to start speech recognition via IPC (on)", { error: error.message });
      }
    });
    ...
    ipcMain.handle("resize-llm-window", (event, { width, height }) => {
      const llmWindow = windowManager.getWindow("llmResponse");
      if (llmWindow && !llmWindow.isDestroyed()) {
        llmWindow.setSize(Math.round(width), Math.round(height));
        if (windowManager.bindWindows) {
          windowManager.positionBoundWindows();
        }
        logger.debug("LLM window manually resized", { width, height });
      }
      return { success: true };
    });

    ipcMain.handle("resize-chat-window", (event, { width, height }) => {
      const chatWindow = windowManager.getWindow("chat");
      if (chatWindow && !chatWindow.isDestroyed()) {
        chatWindow.setSize(Math.round(width), Math.round(height));
        logger.debug("Chat window manually resized", { width, height });
      }
      return { success: true };
    });
